...
def scrape_product_details(url: str, driver=None) -> dict:
    ...
    return _scrape_product_details_fallback(url, driver)
    try:
...
def _scrape_product_details_fallback(url: str, driver=None) -> dict:
    # Fallback: Playwright Chromium browser rendering
    if driver:
        return _scrape_product_details_core(url, driver)
    else:
        import queue
        import threading
        ...
        def worker_task():
            ...
            from utils.playwright_adapter import get_playwright_driver
            settings = load_settings()
            local_driver = get_playwright_driver(settings)
            try:
                res = _scrape_product_details_core(url, local_driver)
                q.put((True, res))
            except Exception as thread_err:
                q.put((False, thread_err))
            finally:
                try:
                    local_driver.quit()
                except Exception:
...
def _scrape_product_details_core(url: str, driver) -> dict:
    url_lower = url.lower()
    platform = "generic"
    if "amazon" in url_lower:
        platform = "amazon"
    ...
    plugin = RETAILER_PLUGINS.get(platform)
    if not plugin:
        plugin = RETAILER_PLUGINS.get("myntra")  # generic fallback
        
    if plugin:
        return plugin.scrape_details(driver, url)
    else:
        raise ValueError(f"No plugin found for platform: {platform}")
...
def main():
    ...
    if not single_run:
        settings = load_settings()
        if os.environ.get("DISABLE_INTERNAL_API_SERVER") != "true":
