...
export const TopLootCard: React.FC<TopLootCardProps> = ({ deal, onOpenHistory }) => {
  ...
  return (
    <div className="mb-8 p-6 rounded-3xl bg-gradient-to-br from-orange-500/10 via-amber-500/5 to-slate-900/90 dark:from-orange-500/15 dark:via-amber-500/5 dark:to-slate-900 border-2 border-orange-500/40 shadow-2xl relative overflow-hidden backdrop-blur-xl transition-all">
      {/* Background Glow Badge */}
      <div className="absolute top-0 right-0 px-6 py-2 bg-gradient-to-l from-orange-500 to-amber-500 text-white font-extrabold text-xs tracking-widest uppercase rounded-bl-2xl shadow-lg flex items-center gap-1.5">
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 pt-4 md:pt-0">
        ...
        <div className="relative w-40 h-40 sm:w-48 sm:h-48 shrink-0 rounded-2xl bg-white p-3 flex items-center justify-center border border-slate-200 dark:border-slate-800 shadow-md">
          <img
            src={deal.image_url || "https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=300"}
            alt={deal.title}
        ...
        <div className="flex-1 w-full text-left">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-slate-200 text-xs font-mono font-bold uppercase border border-slate-700">
              {deal.platform}
            </span>
            <div className="flex items-center gap-1 text-orange-500 font-mono font-extrabold text-xs">
              <Zap className="w-4 h-4" />
              <span>LOOT SCORE: {deal.deal_score}/100</span>
            </div>
          </div>

          <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white line-clamp-2 leading-snug mb-3">
            {deal.title}
          </h3>
          ...
          <div className="flex items-baseline gap-3 mb-4">
            ...
            <span className="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-mono font-extrabold text-xs border border-emerald-500/20 flex items-center gap-1">
              ...
              {deal.discount}% OFF (Save ₹{savings.toLocaleString()})
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {onOpenHistory && (
              <button
                ...
                className="px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-xs border border-slate-300 dark:border-slate-700 transition-all"
              >
                Inspect Price Chart
              </button>
            )}
