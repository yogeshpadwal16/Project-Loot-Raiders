...
# Create a tar.gz archive including essential directories and files (added 'loot_brain', 'tests', 'web', 'docker')
tar.exe --exclude="*venv*" --exclude="*.db*" -czf $TAR_PATH core dashboard database deal_engine knowledge_base plugins scripts utils config web n8n docker loot_brain tests loot_raiders loot_scraper.py selectors.json settings.json requirements.txt .env ecosystem.config.js check_health.py daily_briefing.py main_briefing.py run_all_tests.py run_full_audit.py
...
if [ ! -d "venv" ]; then
    ...
    ./venv/bin/scrapling install || true
else
    # Always keep dependencies in sync
    ./venv/bin/pip install -r requirements.txt
    ./venv/bin/scrapling install || true
fi
