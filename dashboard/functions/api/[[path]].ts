 ...
 * 
 * Location: dashboard/functions/api/[[path]].ts
 */
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  // 1. Enforce strict route allowlist security check
  ...
  // 2. Resolve backend API base URL from Cloudflare Environment Variables
  ...
  // 3. Construct target URL with preserved path and query parameters
  const targetUrl = `${backendBase}${pathname}${url.search}`;

  // 4. Construct clean forward headers (preserve Content-Type and Authorization)
  ...
  // 5. Build fetch options preserving HTTP method and request body
  ...
  // 6. Execute forward fetch with safe error handling
  try {
    const backendResponse = await fetch(targetUrl, fetchOptions);

    // Build clean response headers
    const responseHeaders = new Headers();
    const allowedResponseHeaders = ["content-type", "cache-control", "authorization"];
