...
# Pattern: "10% instant discount up to ₹1500 on HDFC"
_PCT_RE = re.compile(r"(\d+)\s*%\s*(?:off|discount|instant|cashback)", re.IGNORECASE)
_CAP_RE = re.compile(r"up\s*to\s*₹?\s*([\d,]+)", re.IGNORECASE)
# Pattern: "Flat ₹500 off on ICICI"
_FLAT_RE = re.compile(r"(?:flat|get)\s*₹?\s*([\d,]+)\s*(?:off|discount|cashback)", re.IGNORECASE)


def calculate_effective_bank_price(
    deal_price: int, raw_offer_text: str
) -> tuple[int, str]:
    """
    Parses bank offer text and calculates the effective price after the bank discount.

    Handles two patterns:
      - Percentage: "10% instant discount up to ₹1500 on HDFC"
      - Flat:       "Flat ₹500 off on ICICI Credit Cards"

    Returns (effective_price, offer_summary).
    If no parseable offer is found, returns (deal_price, raw_offer_text).
    """
    ...
    if pct_match:
        ...
        if pct <= 0 or pct > 50:
            # Sanity guard: ignore implausible percentages
...
def get_best_bank_effective_price(
    deal_price: int, bank_offers: list[str]
) -> tuple[int, str]:
    """
    Evaluates all bank offers for a deal and returns the best (lowest) effective price.

    Returns (best_effective_price, best_offer_summary).
    If no offers yield a discount, returns (deal_price, "").
    """
    if not bank_offers:
        return deal_price, ""
